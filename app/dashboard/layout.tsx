...
const ICONS: Record<string, JSX.Element> = {
  ...
  phone: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-5 h-5"><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12" y2="18"/></svg>,
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF]">
      ...
      {sidebarOpen && (
        <div
          className="fixed inset-0 z-40 bg-[#1E3A5F]/50 backdrop-blur-sm lg:hidden"
      ...
      {/* Sidebar */}
      <aside className={cn(
        'fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-[#E8E5DF] transform transition-transform duration-300 lg:translate-x-0',
        sidebarOpen ? 'translate-x-0' : '-translate-x-full'
      )}>
        <div className="flex flex-col h-full">
          {/* Logo */}
          <div className="flex items-center justify-between px-5 h-16 border-b border-[#E8E5DF]">
            ...
            <button
              onClick={() => setSidebarOpen(false)}
              className="lg:hidden p-2 text-[#5A6B7D] hover:text-[#1E3A5F] rounded-lg"
            >
          ...
          <div className="px-4 py-3 border-b border-[#E8E5DF]">
            <button className="w-full flex items-center gap-3 p-2.5 rounded-xl bg-[#F5F3EF] hover:bg-[#EBE8E2] transition-colors">
              <div className="w-8 h-8 rounded-lg bg-[#3366FF]/10 flex items-center justify-center">
              ...
              <div className="flex-1 text-left min-w-0">
                ...
                <div className="text-xs text-[#5A6B7D] capitalize">
                  {organization?.subscription_tier || 'Free'} Plan
                </div>
              </div>
              <svg className="w-4 h-4 text-[#5A6B7D] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ...
                      : 'text-[#5A6B7D] hover:text-[#1E3A5F] hover:bg-[#F5F3EF]'
                  )}
                  onClick={() => setSidebarOpen(false)}
                >
                  {ICONS[item.icon]}
          ...
          {/* User menu */}
          <div className="p-3 border-t border-[#E8E5DF]">
            <div className="flex items-center gap-3 p-2.5 rounded-xl bg-[#F5F3EF]">
              <div className="w-9 h-9 rounded-full bg-[#3366FF] flex items-center justify-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-[#5A6B7D] truncate">
                  {user?.email}
                </div>
              </div>
              <button
                onClick={handleSignOut}
                className="p-2 text-[#5A6B7D] hover:text-[#EF4444] rounded-lg transition-colors"
                title="Sign out"
              >
      ...
      <div className="lg:pl-64">
        {/* Top bar */}
        <header className="sticky top-0 z-30 flex items-center justify-between h-16 px-4 lg:px-6 bg-white/80 backdrop-blur-xl border-b border-[#E8E5DF]">
          <div className="flex items-center gap-4">
            ...
            {/* Search */}
            <div className="hidden sm:flex items-center gap-2.5 px-4 py-2 bg-[#F5F3EF] rounded-xl border border-[#E8E5DF] w-64 lg:w-72">
              <svg className="w-4 h-4 text-[#5A6B7D]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                <circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/>
              </svg>
              <input
                type="text"
                placeholder="Search..."
                className="flex-1 bg-transparent text-sm text-[#1E3A5F] placeholder:text-[#5A6B7D]/60 focus:outline-none"
              />
              <kbd className="hidden lg:inline-flex items-center px-1.5 py-0.5 rounded bg-white text-[10px] text-[#5A6B7D] border border-[#E8E5DF]">
                /
              </kbd>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboard/agents/new">
              <button className="hidden sm:flex items-center gap-2 px-4 py-2 bg-[#3366FF] text-white font-semibold text-sm rounded-xl hover:bg-[#2952CC] transition-all shadow-sm">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            {/* Notifications */}
            <button className="relative p-2 rounded-xl bg-[#F5F3EF] text-[#5A6B7D] hover:text-[#1E3A5F] hover:bg-[#EBE8E2] transition-colors">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
